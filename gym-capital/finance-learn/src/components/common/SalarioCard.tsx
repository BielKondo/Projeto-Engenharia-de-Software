...
export function SalarioCard() {
  ...
  const salvarSalario = () => {
    const v = parseFloat(salarioTemp.replace(/\./g, "").replace(",", "."));
    if (!isNaN(v) && v >= 0) definirSalario(v);
    setEditandoSalario(false);
  };

  const salvarRendas = () => {
    const v = parseFloat(rendasTemp.replace(/\./g, "").replace(",", "."));
    if (!isNaN(v) && v >= 0) definirOutrasRendas(v);
